...
class Collocation_system():
    def __init__(self, fun, bc, fun_jac, bc_jac, n, k, s, x, dtype):
        self.fun = fun
        self.bc = bc
        ...
        self.a = lobatto_coeffs[s]['a']
        self.b = lobatto_coeffs[s]['b']
        self.c = lobatto_coeffs[s]['c']

    def collocation_fun(self, y, p):
        ...
        y_stages = y0[:, None, :] + (y1 - y0)[:, None, :] * self.c[None, :, None] # (m-1) x (s) x n
        y_middle = y_stages[:, 1:-1, :]                                           # (m-1) x (s-2) x n

        f = self.fun(x, y, p)                                                     # n x m
        ...
        def optimize_func(x_middle, y_middle, y0, f0, f1, h):
            ...
            y_middle = y_middle.reshape((self.s-2), self.n)

            # shape ((s-2), (s-2) x n) -> (s-2) x n
            f_middle = np.array([self.fun(x_s, y_s, p) for x_s, y_s in zip(x_middle, y_middle)])

            # (s x n)
            entire_f = np.concatenate((f0.reshape(1, self.n), f_middle, f1.reshape(1, self.n)))

            # (n)[:, np.newaxis] + (s-2 x s) @ (s x n) -> (s-2 x n)
            computed_y = y0[np.newaxis,:] + h * self.a[1:-1, :] @ entire_f
        ...
        # (m-1) x (s-2) x n
        f_middle_n = np.array([
            self.fun(x_m, y_m.T, p).T for x_m, y_m in zip(x_middle, y_middle_n)
        ]) 
        ...
        y1_n = y0[:, None, :] + h[:, None, None] * self.a[-1, :] @ f_n
        y1_n = y1_n[:, 0, :]
    ...
    def estimate_y_jac(self, y, p, f0=None):
        # for now we ignore p
        dy_n_dp = None
        
        yn_fun = lambda y: self.collocation_fun(y, p)[0] # we are only interested in the collocation residual
        ...
        if self.m % 2 == 1: # uneven
            eps1 = np.concatenate((eps[:, 0][:, None], np.repeat(eps[:, 2:-2:2], 2, 1), eps[:, -1][:, None]), axis=1)
            eps2 = np.repeat(eps[:, 1:-1:2], 2, 1)

        if self.m % 2 == 0: # even
            eps1 = np.concatenate((eps[:, 0][:, None], np.repeat(eps[:, 2::2], 2, 1)), axis=1)
            eps2 = np.concatenate((np.repeat(eps[:, 1:-1:2], 2, 1), eps[:, -1][:, None]), axis=1)


        dy_n_dy = np.empty((self.n, self.n, 2, self.m-1), dtype=self.dtype)
        ...
        if test:
            ...
            random_forcing = np.random.randn(self.n, self.m) * 1e-5
    ...
    def estimate_bc_jac(self, ya, yb, p, bc0=None):
        ...
        if k == 0:
            ...
            for i in range(k):
                ...
                p_new[i] += h[i]
                hi = p_new[i] - p[i]
                bc_new = self.bc(ya, yb, p_new)
                dbc_dp[i] = (bc_new - bc0) / hi
            dbc_dp = dbc_dp.T

        return dbc_dya, dbc_dyb, dbc_dp

    def construct_global_jac_indices(self):
    ...
    def construct_global_jac(self, y, p, f, bc0):
        ...
        i_jac, j_jac = self.construct_global_jac_indices()

        if self.fun_jac is None:
            dy_n_dy, dy_n_dp = self.estimate_y_jac(y, p, f)
        else:
            dy_n_dy, dy_n_dp = self.fun_jac(self.x, y, p)


        if self.bc_jac is None:
            dbc_dya, dbc_dyb, dbc_dp = self.estimate_bc_jac(y[:, 0], y[:, -1], p, bc0)
        else:
            dbc_dya, dbc_dyb, dbc_dp = self.bc_jac(y[:, 0], y[:, -1], p)
    ...
    def solve_newton(self, y, p, B, bvp_tol, bc_tol):
        ...
        col_res, y_middle, f, f_middle = self.collocation_fun(y, p)
        bc_res = self.bc(y[:, 0], y[:, -1], p)
        ...
        for iteration in range(max_iter):
            ...
            for trial in range(n_trial + 1):
                ...
                col_res, y_middle, f, f_middle = self.collocation_fun(y_new, p_new)
                bc_res = self.bc(y_new[:, 0], y_new[:, -1], p_new)
                res = np.hstack((col_res.ravel(order='F'), bc_res))
...
def create_spline_orig(y, yp, x, h):
    ...
    print(y.shape, yp.shape, x.shape, h.shape)
...
def solve_bvp(fun, bc, x, y, s=5, p=None, S=None, fun_jac=None, bc_jac=None, tol=1e-3, max_nodes=1000, verbose=0, bc_tol=None):
    ...
    col_sys = Collocation_system(fun_wrapped, bc_wrapped, fun_jac_wrapped, bc_jac_wrapped, n, k, s, x, dtype)
    ...
    while True:
        ...
        col_res, y_middle, f, f_middle = col_sys.collocation_fun(y, p)
        bc_res = bc_wrapped(y[:, 0], y[:, -1], p)
